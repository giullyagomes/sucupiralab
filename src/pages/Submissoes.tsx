...
import { loadSubmissoes, saveSubmissaoFile, deleteSubmissaoFile } from '@/lib/githubStorage'
...
export function Submissoes() {
  ...
  const [submissoes, setSubmissoes] = useState<Submissao[]>(
    isDemoMode
      ? demo.submissoes.map(s => ({ ...s, coluna: COLUNA_REMAP[s.coluna] ?? s.coluna }))
      : []
  )
  const [eventos, setEventos] = useState<SubmissaoEvento[]>(isDemoMode ? demo.eventos : [])
  ...
  async function handleSaveEvento() {
    ...
    } catch (err: any) { toast({ title: 'Erro ao registrar evento', description: err.message, variant: 'destructive' }); return }
